...
export const eventsLang = pgTable("events_lang", {
  ...
  nsId: integer("event_id").references(() => events.id),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").$onUpdate(() => new Date()),
});
...
export const nationalSectionsPositions = pgTable("national_section_positions", {
  ...
  birthDate: timestamp("birth_date", { mode: "date" }).defaultNow(),
  deadDate: timestamp("dead_date", { mode: "date" }).defaultNow(),
